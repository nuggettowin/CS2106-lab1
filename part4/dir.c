...
// PRE: oldtable = original hashtable
//      oldlen = length of original hashtable
//      newlen = length of new hashtable
//      hashfun = hash function
// POST: A pointer to the array which contains the new hashtable.
//       Recall that an array is simply pointer to the first element, and the array consists of pointers to TLinkedList
//       Therefore the return type is naturally a TLinkedList **.
//       All files should be copied over into the correct slots in the new hashtable.

TLinkedList **resize_hashtable(TLinkedList *oldtable[], int oldlen, int newlen, int (*hashfun)(char *, int)) {

    // You can check testdir.c to see how the new hashtable can be initialized using malloc.
}
